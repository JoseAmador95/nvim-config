...
- Reviewing what an out-of-process coding agent (Claude Code, Codex) wrote is a first-party workflow under `<leader>v` (`lua/config/agent_review/`, spec `lua/plugins/agent-review.lua`, health `lua/agent_review/health.lua`; see `docs/agent-review.md` and `docs/adr/0001-agent-review.md`). `<leader>vs` snapshots the worktree into `refs/agent-review/*` **before** the agent runs — that snapshot, not `HEAD`, is the baseline — then `<leader>vv` (risk-ranked dashboard), `<leader>vf` (machine pass: LSP diagnostics + stubs + silent deletions → quickfix), `]v`/`[v` plus `<leader>va/vx/vc` (accept / reject-as-mark-only / comment; verdicts keyed by hunk content hash in `stdpath("state")`), `<leader>vy` (prompt in a buffer and in `+`). `:checkhealth agent_review`; per-host options under `agent_review` in `~/.nvim-local.lua`.
